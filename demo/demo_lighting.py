...
def main(args):
    ...
    # generate camera views and lighting locations (assume same lighting energy)
    img_hw = (img_h, img_w)
    ...
    K = sdf_renderer.get_intrinsic()
    azimuth_deg = 60
    view = View(azimuth_deg, 25, 0, 2.5)
    RT = view.get_extrinsic()
    camera = Camera(K, RT)
    NUMBER = 180
    lighting_src_list = []
    for idx in range(NUMBER):
        azimuth_deg = idx * 360 / NUMBER
        view = View(azimuth_deg, 25, 0, 2.0)
        lighting_loc = view.get_camera_pos()
        lighting_src_list.append(np.array([lighting_loc[0], lighting_loc[1], lighting_loc[2]]))
    ...
    if not os.path.exists(basedir):
        ...
        os.makedirs(basedir)
    for idx, lighting_loc in enumerate(tqdm(lighting_src_list)):
        prefix_idx = prefix + '_{0:04d}'.format(idx)
        demo_color_save_render_output(prefix_idx, sdf_renderer, shape_code, color_code, camera, lighting_loc=lighting_loc)
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument('--class_name', type=str, default='lamp', help='class name.')

    arg_parser.add_argument('-i', '--instance_id', type=int, default=330, help='instance id.')
    arg_parser.add_argument('--vis_folder', type=str, default='vis/demo_lighting', help='folder for visualization')
